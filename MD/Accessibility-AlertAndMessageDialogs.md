...
## Behavior
1. Interrupts the user's workflow to communicate an important message and acquire a response
...
2. <b> `aria-modal=true` should be set to the dialog dom element to tell assistive technologies that the windows underneath the current alert dialog are not available for interaction (inert) </b>
...
<button type="button" aria-controls="notes" id="notes_save">
  Save
</button>
<button
  type="button"
...
<div role="alert" id="alert_toast" class="toast hidden">
  Nothing to discard.
</div>
